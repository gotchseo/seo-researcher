  ...
  const fetcher=vi.fn(async (url: string, init: RequestInit) => {
    ...
    if (body) {
      ...
      if (body.method==='tools/list') {
        ...
        return Response.json({result:{tools:['seo_research_connection','seo_research_usage','seo_research_start','seo_research_status','seo_research_get','seo_research_list'].map(name=>({name}))}});
      }
